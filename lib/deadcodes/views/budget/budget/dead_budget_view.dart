...
import 'dead_budget_details_screen.dart';


class DeadBudgetView extends StatefulWidget {
  const DeadBudgetView({Key? key}) : super(key: key);

  @override
  _DeadBudgetViewState createState() => _DeadBudgetViewState();
}


class _DeadBudgetViewState extends State<DeadBudgetView> {
    ...
    if (user != null) {
      ...
      if (budgetSnapshot.docs.isEmpty) {
        ...
        if (previousBudgetSnapshot.docs.isNotEmpty) {
          ...
          final newBudget = DeadBudgetModel(
            ...
            categories: (previousBudget['categories'] as List)
                .map((category) => DeadCategoryModel.fromMap(category))
                .toList(),
          );
          ...
          await dead_copyRecurringTransactions(previousBudget.id, newBudget.id);
        }
      }
    }
  }
                    ...
                    return ListTile(
                      ...
                      onTap: () {
                        Navigator.push(
                          context,
                          MaterialPageRoute(
                            builder: (context) => DeadBudgetDetailsScreen(budgetId: budget.id),
                          ),
                        );
                      },
                    );
