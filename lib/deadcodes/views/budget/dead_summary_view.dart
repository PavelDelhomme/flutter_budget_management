...
import '../../services/dead_income_service.dart';  // Importation pour générer des couleurs aléatoires

class DeadSummaryView extends StatefulWidget {
  const DeadSummaryView({Key? key}) : super(key: key);

  @override
  _DeadSummaryViewState createState() => _DeadSummaryViewState();
}

class _DeadSummaryViewState extends State<DeadSummaryView> {
  ...
  Future<Map<String, dynamic>?> _getBudgetSummary() async {
    ...
    if (user != null) {
      try {
        ...
        final incomes = await dead_getUserIncomes(user.uid, DateTime.now().month, DateTime.now().year);
