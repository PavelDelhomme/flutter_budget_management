...
Future<void> dead_addTransaction({
    ...
    if (budgetDoc.exists) {
      ...
      if (categoryIndex != -1) {
        ...
        if (useSavings && savingCategoryId != null) {
          ...
          double remainingSavings = await dead_deductFromSavings(user.uid, savingCategoryId, amount);

          if (remainingSavings > 0) {
            // Mettre à jour le budget avec le montant retiré des économies
            await _dead_updateMonthlyBudget(remainingSavings, budgetId);
          }
        }
...
Future<double> dead_deductFromSavings(String userId, String savingId, double amountToDeduct) async {
...
Future<void> _dead_updateMonthlyBudget(double amount, String budgetId) async {
  final budgetRef = FirebaseFirestore.instance.collection('budgets').doc(budgetId);
  final budgetDoc = await budgetRef.get();
