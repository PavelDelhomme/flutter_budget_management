...
import '../../../dead_utils.dart';
import '../../../models/dead_budget.dart';
import '../../../models/dead_category.dart';
import '../../../services/dead_income_service.dart';
import '../../navigation/dead_tab_navigation.dart';


class DeadAddBudgetScreen extends StatefulWidget {
  const DeadAddBudgetScreen({Key? key}) : super(key: key);

  @override
  _DeadAddBudgetScreenState createState() => _DeadAddBudgetScreenState();
}


class _DeadAddBudgetScreenState extends State<DeadAddBudgetScreen> {
  ...
  List<DeadCategoryModel> _categories = [];
  ...
  Future<void> _loadUserIncome() async {
    ...
    if (user != null) {
      try {
        final incomes = await dead_getUserIncomes(user.uid, _selectedMonth, _selectedYear);
  ...
  Future<void> _checkAndCopyRecurringIncomes() async {
    ...
    if (user != null) {
      ...
      try {
        final incomes = await dead_getUserIncomes(user.uid, previousMonth, previousYear);
  ...
  void _addCategory() {
    ...
    if (categoryName.isNotEmpty && categoryAmount > 0 && categoryAmount <= _remainingIncome) {
      setState(() {
        _categories.add(DeadCategoryModel(name: categoryName, allocatedAmount: categoryAmount));
        _remainingIncome -= categoryAmount;
        _categoryNameController.clear();
        _categoryAmountController.clear();
            ...
            TextButton(
              onPressed: () {
                ...
                if (newName.isNotEmpty && newAmount > 0 && newAmount <= (_remainingIncome + category.allocatedAmount)) {
                  setState(() {
                    _remainingIncome += category.allocatedAmount - newAmount;
                    _categories[index] = DeadCategoryModel(name: newName, allocatedAmount: newAmount);
                  });
                } else {
                  ScaffoldMessenger.of(context).showSnackBar(
  ...
  Future<void> _createBudget() async {
    ...
    if (user != null && _categories.isNotEmpty) {
      ...
      final budget = DeadBudgetModel(
        id: dead_generateBudgetId(),
        userId: user.uid,
        description: description,
        totalAmount: _totalIncome - _remainingIncome,
        savings: _remainingIncome,
      ...
      Navigator.of(context).pushReplacement(
        MaterialPageRoute(builder: (context) => DeadTabNavigation(budgetId: budget.id)),
      );
    } else {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Veuillez ajouter au moins une catégorie.')),
