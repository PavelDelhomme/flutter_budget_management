...
import 'dead_transaction_details_modal.dart';
import 'dead_transaction_form_screen.dart';

class DeadTransactionsView extends StatefulWidget {
  final String? budgetId;

  const DeadTransactionsView({Key? key, this.budgetId}) : super(key: key);

  @override
  _DeadTransactionsViewState createState() => _DeadTransactionsViewState();
}

class _DeadTransactionsViewState extends State<DeadTransactionsView> {
  ...
  void _editTransaction(BuildContext context, DocumentSnapshot transaction) async {
    ...
    if (budgetId != null) {
      final result = await Navigator.push(
        context,
        MaterialPageRoute(
          builder: (context) => DeadTransactionFormScreen(budgetId: budgetId, transaction: transaction),
        ),
      );
                        ...
                        showModalBottomSheet(
                          ...
                          builder: (context) {
                            return Padding(
                              ...
                              child: DeadTransactionDetailsModal(transaction: transaction),
                            );
                          },
                        );
                      },
        ...
        onPressed: () async {
          ...
          if (selectedBudgetId != null) {
            Navigator.push(
              context,
              MaterialPageRoute(
                builder: (context) => DeadTransactionFormScreen(budgetId: selectedBudgetId),
              ),
            );
          } else {
            ScaffoldMessenger.of(context).showSnackBar(
